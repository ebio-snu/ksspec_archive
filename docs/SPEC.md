...
#### `meaning-note` — 코드의 적용 범위를 좁히는 근거 (device code 12 · 16)

`meaning` 은 KS B 7958-5 부속서 A.2 의 표기를 그대로 옮긴 것이고, `meaning-note` 는
**다른 표준이 그 코드의 적용 범위를 이미 좁혀 둔 경우** 그 근거를 적는 자리다. 코드값이나
이름을 바꾸지 않으며, 소비처가 코드를 잘못 넓게 해석하는 것을 막는다.

| 코드 | 매질 | 근거 |
|---|---|---|
| device 12 (`EC-sensor`) | **수용액** | KS X 3269:2022 §3.1 |
| device 16 (`pH-sensor`) | **수용액** | KS X 3269:2022 §3.1 |

토양·배지에 꽂힌 센서가 내는 값은 **벌크 EC** 로 수용액 EC 와 다른 물리량이다. **단위가
둘 다 dS/m 이라 섞여도 드러나지 않으므로**, 코드만으로 매질을 판정하면 안 된다
(`codes.json` 의 `interop-notes`). 매질별 코드 분화는 제정 협의 대상이며 신규 코드 번호는
미정이다.
